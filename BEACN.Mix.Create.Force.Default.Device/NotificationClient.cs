...
using BEACN.Mix.Create.Force.Default.Device.Com;
...
namespace BEACN.Mix.Create.Force.Default.Device
{
    public class NotificationClient : IMMNotificationClient
    {
        ...
        private readonly Dictionary<string, MMDevice> _BEACNDevices = new Dictionary<string, MMDevice>();
        ...
        public NotificationClient()
        {
            ...
            {
                if (!IsBEACN(device)) //Skip all BEACN devices.
                    continue;

                //DeviceId:
                //{0.0.0.00000000} -> Renderer
                //{0.0.1.00000000} -> Capture
                _BEACNDevices[device.ID] = device;
        ...
        private void SetDefaultAudioDevice(DataFlow flow, Role role, string newDefaultDeviceId, string searchString)
        {
            ...
            if (newDefaultDeviceId == device.ID)
                return; //New default device is the correct BEACN device, ignore.
        ...
        {
            if (!_BEACNDevices.ContainsKey(deviceId))
                return;
        ...
        {
            if (!_BEACNDevices.ContainsKey(pwstrDeviceId))
                return;
        ...
        private bool IsBEACN(MMDevice device)
        {
            try
            {
                return device.DeviceFriendlyName
                    .IndexOf("BEACN Mix Create", StringComparison.OrdinalIgnoreCase) >= 0;
            }
            catch (COMException)
            {
                return false;
